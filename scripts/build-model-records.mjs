...
say('ищу новые модели...');
{
  ...
  /*
   * Карта свёрнутых нужна ЗДЕСЬ, а не только при сборке семей.
   *
   * Модель, свёрнутая в чужую карточку, своей страницы иметь не должна. Папки
   * у неё после склейки нет, и по одному лишь наличию папки адрес выглядит
   * свободным - сборщик заводил запись заново, а генератор возвращал страницу.
   * Так на сайте появились бы дубли: и семья, и её вариант отдельной карточкой.
   * Поймано на 46 адресах.
   */
  const mergedFile = path.join(DATA, 'merged-variants.json');
  const merged = fs.existsSync(mergedFile)
    ? JSON.parse(fs.readFileSync(mergedFile, 'utf8')) : {};

  let added = 0, already = 0, taken = 0, folded = 0;
  if (fs.existsSync(f)) {
    ...
    for (const p of np) {
      ...
      // Уже свёрнута в чужую карточку - своей страницы не заводим.
      if (merged[slug]) { folded++; continue; }
  ...
  say('новых моделей к созданию: ' + added
    ...
    + (folded ? ', свёрнуты в чужую карточку: ' + folded : '')
...
say('читаю product-report.json...');
{
  ...
  for (const p of rep) {
     ...
     */
    const fromReport = normCert(p.cert);
    const fromExcel = r.csv_cert && !/Lite\/Pro/i.test(r.csv_cert) ? normCert(r.csv_cert) : '';
    /*
     * СЕРТИФИКАТ СНИМАТЬ НЕЛЬЗЯ - решение основателя от 02.09.2026.
     *
     * Отчёт новее и остаётся хозяином графы: он повышает («нет» -> CheckMate
     * Pro, StemCell -> Pro) и заполняет пустое. Но там, где он говорит «нет
     * сертификата» про модель, у которой в Excel стоит StemCell, значок
     * остаётся. Это 762 карточки.
     *
     * Почему так. Программу StemCell не закрывали - её несут 11 702 модели в
     * том же свежем отчёте. Значит «no certification» у этих 762 - скорее
     * пробел выгрузки, чем отзыв сертификата. Цена ошибки несимметрична: снять
     * значок легко, вернуть - только новой выгрузкой, а до тех пор карточка
     * недоговаривает о товаре, который сертификат имеет.
     */
    const excelKeepsStemCell = fromExcel === 'StemCell' && fromReport === 'no certification';
    r.cert = certSilent(p.cert) && fromExcel ? fromExcel
      : excelKeepsStemCell ? 'StemCell'
        : fromReport;
    r.date = p.date || '';
...
say('читаю категории и отрасли...');
{
  ...
  for (const r of byId.values()) {
     ...
     */
    /*
     * Категория: единый источник, запасной - отчёт TurboSquid.
     *
     * ТРЕТИЙ РАЗ ОДНА И ТА ЖЕ ОШИБКА, поэтому пишу подробно. Здесь стояло
     * условие «спрашивать отчёт только если модель новая». Состояние `new`
     * считается по отсутствию папки, поэтому оно живёт ровно до первой сборки
     * страниц: как только страницы созданы, модель перестаёт быть новой - и
     * запасной путь отключается. Категория 581 карточки схлопнулась в «Other».
     *
     * Так же сломались снимки студии («только новым») и раньше - галереи
     * («у кого нет»). Признак «новая» НЕ ГОДИТСЯ как метка происхождения
     * данных: он про наличие папки, а не про то, откуда брать значение.
     *
     * Верное условие - «в едином источнике этой модели ещё нет». Оно не
     * протухает и работает одинаково до и после сборки страниц.
     */
    r.category = cat[r.id] || classifyByReport(r.id, r.name || '') || 'other';
...
say('читаю выгрузку студии...');
{
  ...
  for (const f of fs.readdirSync(DL)) {
    ...
    for (const [id, s] of Object.entries(j.result || {})) {
      ...
      /*
       * Снимки из выгрузки студии. Для НОВОЙ модели это единственный источник:
       * выгрузка каталога TurboSquid обрывается на ID 2587532, а всё, что вышло
       * позже, там просто отсутствует. Без этого карточка собиралась бы с
       * заглушкой вместо превью.
       *
       * Главным ставим кадр с окончанием _001: у студии это общий план, с
       * которого начинают показ. Остальные идут в галерею собственных снимков -
       * ровно тем же полем, что и у старых карточек, снятое со страниц.
       */
      if (Array.isArray(s.images) && s.images.length) {
         ...
         */
        /*
         * ВЫБОР КАДРА. Имя студийного файла кончается порядковым номером:
         * ..._001.jpg, ..._026.jpg. Общий план у студии идёт первым, а
         * технические ракурсы - сетка, развёртка, каркас - в конце.
         *
         * Здесь стояла сортировка строк целиком. Она сравнивала не номер, а
         * начало имени файла, то есть случайный идентификатор, и в плитку
         * попадал произвольный кадр. Так на витрине оказались каркасы:
         * Sikorsky UH-60 брал _026, Blimp Goodyear - _022, Sea Shell - _011.
         * Покупатель видел чертёж вместо предмета.
         *
         * Сравниваем сам номер, числом. Кадра без номера быть не должно, но
         * если он такой - отправляем в конец, а не в начало.
         */
        const shotNo = u => {
          const m = String(u).match(/_(\d+)\.[a-z]+$/i);
          return m ? Number(m[1]) : Number.MAX_SAFE_INTEGER;
        };
        const first = [...s.images].sort((a, b) => shotNo(a) - shotNo(b)
 ...
 * человеком текста: раньше он жил ТОЛЬКО в файле страницы, помеченный
 * <!-- written:v1 -->, и при переходе карточек на сборку из записи был затёрт
 * заготовкой - 1 786 карточек. Восстановлен из истории git и положен сюда,
 * чтобы больше никакая пересборка его не тронула.
 *
 * Файл под присмотром git, в отличие от остальных data/model-*.json: их можно
 * снять со страниц заново, а этот - нельзя, если история потеряется.
 */
const HAND_DESC = new Map(Object.entries(
  fs.existsSync(path.join(DATA, 'model-hand-desc.json'))
    ? JSON.parse(fs.readFileSync(path.join(DATA, 'model-hand-desc.json'), 'utf8'))
    : {}));
...
say('читаю карту свёрнутых...');
{
  ...
  for (const r of byId.values()) {
     ...
     * уезжает в подписи блока Related соседних карточек - оттуда дыра и
     * попадала на страницы, где сама модель ни при чём.
     */
    r.display_name = String(r.display_name).replace(/\s+/g, ' ').trim();
  }
}
...
/*
 * Снимки студии - там, где своих нет.
 *
 * Ставим ПОСЛЕ всех прочих источников, чтобы не перебить ни выбранный вручную
 * главный кадр, ни галерею, снятую со страницы. Для существующих карточек эти
 * поля уже заполнены, и сюда попадают только новые модели.
 *
 * Главным берём кадр с окончанием _001: у студии это общий план. Если такого
 * нет - первый по порядку имени, а не случайный: порядок должен быть один и
 * тот же при каждом прогоне, иначе сборка перестанет быть воспроизводимой.
 */
{
  let heroed = 0, galleried = 0;
  for (const r of byId.values()) {
    /*
     * УСЛОВИЕ - «превью нет вовсе», а не «модель новая».
     *
     * Две ошибки подряд в этом месте, обе поучительные.
     *
     * Сначала я поставил условие «нет своей галереи» - и добавил студийные
     * снимки 47 405 существующим карточкам: у них превью было, а галереи не
     * было, и они все попали под правило.
     *
     * Потом сузил до «модель новая» - и правило перестало работать ровно
     * тогда, когда понадобилось: как только новым моделям собрали страницы,
     * они перестали быть новыми, и на следующей сборке остались без превью.
     * Состояние `new` считается по наличию папки, поэтому оно НЕ годится как
     * признак «эта модель из студийной партии».
     *
     * Верное условие - отсутствие превью. У всех существующих карточек оно
...
// studio_images - рабочее поле: из него уже выбраны главный кадр и галерея,
// хранить его в записи значило бы держать те же адреса дважды.
const clean = r => { const { csv_cert, csv_category, report_name, report_price, excel_name, excel_price, studio_images, ...keep } = r; return keep; };
let chunks = 0;
