...
NAV = '''<nav style="background:rgba(7,9,15,0.95);border-bottom:1px solid rgba(255,255,255,0.07);position:sticky;top:0;z-index:50;">
  <div style="max-width:1200px;margin:0 auto;padding:0 24px;height:60px;display:flex;align-items:center;gap:32px;">
    <a href="/3D-Models/" style="font-family:'Playfair Display',serif;font-weight:800;font-size:18px;color:#fff;text-decoration:none;letter-spacing:-0.02em;">3D <span style="color:#00E5C4;">Molier</span></a>
...
<main style="max-width:1200px;margin:0 auto;padding:48px 24px 80px;">
  ...
  {'<div class="model-grid">' + models_html + '</div>' if models_html else '<p style="color:#9CA3AF;">Browse all models in the <a href="/3D-Models/catalog/" style="color:{color};">full catalog</a>.</p>'.replace('{color}', color)}
  ...
  <div style="margin-top:48px;padding:32px;background:#0D1117;border-radius:14px;border:1px solid rgba(255,255,255,.07);">
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;">
      ...
      <a href="/3D-Models/search/" class="btn-ghost">Search Models</a>
      <a href="/3D-Models/custom-order/" class="btn-ghost">Request Custom</a>
    </div>
  </div>
...
def create_industry_page(slug, meta):
    ...
    cat_links_html = "".join(
        f'<a href="/3D-Models/categories/{cat_slug}/" style="color:{color};text-decoration:none;font-weight:600;display:block;margin-bottom:6px;">{cat_name} 3D Models</a>'
