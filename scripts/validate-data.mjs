...
/*
 * ── 10. блока «нет результатов» в разметке быть НЕ ДОЛЖНО ──
 *
 * Правило перевёрнуто, и это важно понимать, иначе следующая правка вернёт
 * старое. Раньше проверка требовала, чтобы блок ЛЕЖАЛ в разметке с атрибутом
 * hidden. Это было ошибкой: hidden убирает элемент с экрана, но не из
 * документа. В исходнике страницы под полусотней карточек стояло утверждение,
 * что моделей не найдено, и счёт из двух нулей - и всё, что читает разметку, а
 * не картинку, получало противоречие.
 *
 * Теперь оба блока создаются скриптом и только тогда, когда есть что сказать.
 * Проверка стережёт, чтобы они не вернулись в статический HTML.
 */
{
  const cat = fs.readFileSync(path.join(ROOT, 'catalog', 'index.html'), 'utf8');
  const body = cat.replace(/<!--[\s\S]*?-->/g, '').replace(/<script[\s\S]*?<\/script>/g, '');
  if (/id="empty"/.test(body)) {
    fail(10, 'блок «нет результатов» вернулся в разметку каталога - его должен создавать скрипт', []);
  }
  if (/id="fc-progress"/.test(body) || /of\s*<span id="fc-total">/.test(body)) {
    fail(10, 'строка счётчика вернулась в разметку каталога - её должен создавать скрипт', []);
  }
}
{
...
/*
 * 14. Каталог не должен ни показывать, ни ОБЪЯВЛЯТЬ в разметке, что моделей
 * нет, когда они есть.
 *
 * Прежняя редакция требовала атрибут hidden и считала, что этого довольно. Не
 * довольно: hidden убирает узел с экрана и из дерева доступности, но текст
 * остаётся в документе, и любой разбор разметки его читает. Правильное правило -
 * такого текста в статическом HTML нет вовсе, его пишет скрипт по факту.
 */
{
  const bad = [];
  ...
  if (fs.existsSync(f)) {
    const h = fs.readFileSync(f, 'utf8');
    const body = h.replace(/<!--[\s\S]*?-->/g, '').replace(/<script[\s\S]*?<\/script>/g, '');
    ...
    if (/No models found/i.test(body)) bad.push('в разметке каталога есть «No models found»');
    if (/Showing[\s\S]{0,40}0[\s\S]{0,20}models/i.test(body)) bad.push('в разметке каталога есть счёт из нулей');
    console.log('  [14] каталог: карточек в разметке ' + cards
      + ', утверждений о пустой выдаче: ' + bad.length);
  }
