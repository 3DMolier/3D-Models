...
const parseDetails = d => {
  const s = String(d || '');
  // Размеры текстур ищем ТОЛЬКО до слова Dimensions. Иначе в них попадают
  // куски габаритов: из «10,24 x 11,78 x H4,94 m» выходило «24x11», и такой
  // «размер текстуры» уезжал и в карточку, и в образец данных для клиентов.
  // Мусором оказалась треть значений - 891 из 2 749.
