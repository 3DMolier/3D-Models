 ...
 */
/*
 * Значение берём ВМЕСТЕ С РАЗМЕТКОЙ внутри ячейки, а теги снимаем сами.
 *
 * Прежний вариант читал текст сразу после <td> и останавливался на первом «<».
 * Когда сертификат обернули ссылкой на /model-standards/, первым символом стал
 * «<a», и проверка начала возвращать пустую строку - на ВСЕХ 37 683 карточках.
 * Отчёт при этом выглядел спокойно: «строки сертификации нет», «совпало 0», ни
 * одной ошибки. Ослепшая проверка опаснее отсутствующей: она создаёт ощущение,
 * что всё сверено.
 */
const onPage = html => {
  const m = html.match(/(?:Certification|Quality standard)<\/(?:th|dt|span)>\s*<(?:td|dd|span)[^>]*>([\s\S]*?)<\/(?:td|dd|span)>/i);
  if (!m) return '';
