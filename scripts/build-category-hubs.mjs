  ...
  if (fs.existsSync(idxFile)) {
    ...
    for (let k = 0; k < idx.chunks; k++) {
      for (const r of JSON.parse(fs.readFileSync(path.join(RECS, 'records-' + k + '.json'), 'utf8'))) {
        ...
        /*
         * Для плитки берём image, а thumb - только если image нет.
         *
         * Раньше было наоборот, и причина была разумной: в image лежит кадр
         * TurboSquid 1920x1080 весом 171 КБ, а у студийного снимка есть
         * готовые копии h200 и h400. Экономия на трафике настоящая.
         *
         * Но студийный хост оказался слишком медленным и неровным: один и тот
         * же адрес то отвечает за секунду, то висит две минуты и срывается, а
         * уменьшенные копии вдобавок отдают 415 там, где оригинал отдаёт 502.
         * На странице категории это выглядело как пустые плитки с заглушкой -
         * при том, что на самой карточке снимок открывался.
         *
         * Кадр TurboSquid лежит на их CDN и отдаётся всегда. Тяжелее - да, но
         * картинки грузятся лениво, а показанная тяжёлая картинка полезнее
         * ненайденной лёгкой.
         */
        if (r.image || r.thumb) img[r.id] = r.image || r.thumb;
