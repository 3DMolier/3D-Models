...
export function toContentFields(r) {
  ...
  return {
    ...
    cert: r.cert || '',
    /*
     * Марка нужна тексту, а не только строке лицензии: у брендовой модели
     * лицензия редакционная, и фраза о применении не имеет права называть
     * рекламу. Без этого поля card-content вынужден был угадывать марку по
     * названию заново.
     */
    brand: r.brand || null,
 ...
 */
/*
 * УМЕНЬШИТЕЛЬ ВЫКЛЮЧЕН. Один флаг, чтобы вернуть - одной строкой.
 *
 * Сервис копий на студии сломан. Замер 05.09.2026, по одному запросу с
 * повтором:
 *     оригинал /assets/        95-98% отдаётся
 *     копия /images/h400/      18%
 *     копия /images/h200/      30%
 * Отказ приходит не ошибкой, а СРЫВОМ соединения - сервер молчит до
 * таймаута. Поэтому запасной адрес в onerror не спасает: на зависание
 * обработчик не срабатывает, и посетитель видит пустую рамку сколь угодно
 * долго.
 *
 * Решение основателя: пустая рамка хуже медленной картинки, а починка может
 * занять недели. Пока сервис не работает, отдаём оригиналы - они тяжелее, но
 * они видны.
 *
 * ЧТОБЫ ВЕРНУТЬ: поставить RESIZER_WORKS = true, пересобрать записи и
 * карточки. Адреса копий строятся тем же кодом, менять больше нечего.
 * Такой же флаг есть в build-category-hubs.mjs - переключать оба вместе.
 */
